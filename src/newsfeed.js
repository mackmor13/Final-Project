...
var _FIREBASE = {
  ...
  "users" : {
    "ETP9mIUpPZTqGS1st5yp3ToIKpz1" : {
      "preferredCategory" : "UnitedKingdom",
      "preferredFeed" : "bbcnews"
    }
  },
  "wiredde" : {
    "category" : "technology",
...
class NewsFeed extends React.Component {
    ...
    constructor(props) {
        ...
        super(props)
            ...
            this.state = {feedData:null, categorySelected: null,feedSelected:null , moodSelected: "all"};
    ...
    changeFeed () {
        ...
        //Should change the article feed in th the card groups

        //Should use fetchData to get the latest object of news articles
        

        //Check if there are any new news articles by matching to the unique ids
        //If there are new articles add them to the unique id list and the data base otherwise do nothing 
        //(remember to add keys - city,happy,sad,angry,wow,neutral)

        //Set the feedData state to call articles 

    }

    fetchData(stationID, sortOption) {
        var thisComponent = this; //work around for scope!
        DataController.searchData(stationID, sortOption)
            .then(function (data) {
                thisComponent.setState({ feed: data.items })
            })
            .catch((err) => this.setState({ feed: []}));
...
class HorizontalNavigation extends React.Component {
    ...
    render () {
        ...
        var categories = _FIREBASE.categories;
